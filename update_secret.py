...
def main():
    ...
    if not os.path.exists(USERS_FILE):
        print("ℹ️ users.txt не найден — обновление секрета пропущено.")
        return

    with open(USERS_FILE, "r", encoding="utf-8") as f:
        users = sorted({line.strip() for line in f if line.strip()})

    if not users:
        print("ℹ️ Список пользователей пуст — обновление секрета пропущено.")
        return
    ...
    with urllib.request.urlopen(req, timeout=30) as resp:
        print(f"✅ Секрет {SECRET_NAME} обновлён: пользователей: {len(users)}")
